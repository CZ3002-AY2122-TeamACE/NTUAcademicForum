<template>
  <div class="item">
    <div class="ui comments">

      <div class="ui container">
        <div class="comment">
          <a class="avatar">
            <img src="../assets/default profile.jpeg">
          </a>
          <div class="content">
            <a class="author">{{reply.user_id}}</a>
            <i class="calendar icon"></i>{{reply.created_at}}
            <div class="text">
              {{reply.content}}
            </div>
            <div class="actions">
              <button type="button" class="close" aria-label="Close"><i class="thumbs up outline icon"></i></button>
              <button type="button" class="close" aria-label="Close"><i class="thumbs down outline icon"></i></button>
              <button v-b-toggle="'collapse-1'" type="button" class="close" aria-label="Close">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat-left-text-fill" viewBox="0 0 16 16">
                  <path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H4.414a1 1 0 0 0-.707.293L.854 15.146A.5.5 0 0 1 0 14.793V2zm3.5 1a.5.5 0 0 0 0 1h9a.5.5 0 0 0 0-1h-9zm0 2.5a.5.5 0 0 0 0 1h9a.5.5 0 0 0 0-1h-9zm0 2.5a.5.5 0 0 0 0 1h5a.5.5 0 0 0 0-1h-5z"/>
                </svg></button>
              <b-collapse id="collapse-1">
                <!--        <b-card>I am collapsible content!</b-card>-->
                <post-editor></post-editor>
                <b-button v-b-toggle="'collapse-1'" class="m-1" v-on:click="postReply">Submit</b-button>
              </b-collapse>
            </div>
          </div>

          <!--<div class="content display-reply">
            <a>show 3 replies<i class="angle down icon"></i></a>
          </div>-->
        </div>
    </div>

    </div>
  </div>
</template>
<script>
import PostEditor from "@/components/PostEditor";
import main from "../main";
export default {

name: "Reply",
  props:['reply'],
  components:{
    PostEditor,
  },
  data(){
    return{
      showReply:false,
    }
  },
  methods: {

    postReply: function() {
      var replyTo = null;
      this.errors = [];
      if(this.$store.state.thread.content== "") {
        this.errors.push('Reply content is empty');
      }

      main.addReply(this.$store.state.key, this.$store.state.thread.content, this.$store.state.currentUser.id, replyTo);

    }},
}
</script>

<style scoped>

.display-reply {
  margin-top: 5px;
}
</style>